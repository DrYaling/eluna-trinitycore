...
bool WaypointMovementGenerator<Creature>::StartMove(Creature* creature)
{
	...
	Movement::Location formationDest(node->x + (float)irand(4, 6), node->y + (float)irand(4, 6), node->z, 0.0f);
	...
	init.MoveTo(node->x + (float)irand(4, 6), node->y + (float)irand(4, 6), node->z);
    ...
    if (creature->GetFormation() && creature->GetFormation()->getLeader() == creature)
		creature->GetFormation()->LeaderMoveTo(formationDest.x + irand(4, 6), formationDest.y + irand(4, 6), formationDest.z);
