...
bool Unit::IsAnyNpcNearby(float dist) 
{
	bool	Npc_nearby=false;
    ...
    for (std::list<Unit*>::iterator tIter = targets.begin(); tIter != targets.end();)
    {
		...
		{
			if(!(*tIter)->IsPet() && !( (*tIter)->getRace()>0 && (*tIter)->getRace()<11 ) && this->IsValidAttackTarget(*tIter))
			{
				Npc_nearby=true;
				break;
			}
...
Unit* Unit::SelectnearbyPTarget(uint32 team,float dist)
{
    std::list<Unit*> targets;
    Trinity::AnyUnfriendlyUnitInObjectRangeCheck u_check(this, this, dist);
    Trinity::UnitListSearcher<Trinity::AnyUnfriendlyUnitInObjectRangeCheck> searcher(this, targets, u_check);
    ...
    for (std::list<Unit*>::iterator tIter = targets.begin(); tIter != targets.end();)
    {
		...
		{
			if((*tIter)->GetVictim())					//可能有无目标的对象
			{
				if((*tIter)->GetVictim()->GetTypeId()==TYPEID_PLAYER)
				{
					uint32 race=(*tIter)->GetVictim()->getRace();
					if(race==1 || race==3 ||race==4 ||race==7 ||race==11)		//联盟玩家
					{
						if(team==TEAM_ALLIANCE)
							return	*tIter;
					}
					else if(race>0 && race<=10)						//部落玩家
					{
						if(team == TEAM_HORDE)
							return *tIter;
					}
